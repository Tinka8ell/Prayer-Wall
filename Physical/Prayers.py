...
class Prayer:
   ...
   def __init__(self, number, subject, prayer, author=None, count=0, responses=None):
      self._number = number
   ...
   def prayer(self):
      return [self.number(), self.subject(), self.prayerText(), self.author(), self.count(), self.responses()]
...
class Prayers:
   ...
   def summary(self):
      ...
      if time.time() - self.lastExtract > 10: # refresh every 10 seconds!
         ...
         for number in self.listOfPrayers.keys():
            ...
            content = [number, prayer.subject(), str(min(9, prayer.count()))]
   ...
   def loadPrayer(self, key, value):
      ...
      prayer = Prayer(key, subject, prayer, author=author, count=count, responses=responses)
...
if __name__ == '__main__':
   ...
   filename = "/tmp/" + datetime.now().isoformat(timespec='seconds').replace(":", "-") + ".json"
   ...
   for number in prayers.listOfPrayers:
      ...
      print(prayer.number(), prayer.subject(), "text", prayer.author(), prayer.count(), prayer.responses(), sep='\t')
