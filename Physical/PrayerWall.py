...
from tkinter import Tk, Button, Frame, Text, Toplevel, Canvas, PhotoImage
from tkinter import Label, IntVar, StringVar, LabelFrame, messagebox, Scrollbar
...
def newRequest(parent):
    ...
    dialog = PrayerDialog(
        parent, title="New Prayer Request", mustHaveSubject=True)
    if dialog.result:
        ...
        if ok:
            messagebox.showinfo("Pray4Chichester",
                                "Thank you for your prayer", 
                                parent=parent)
        else:
            messagebox.showerror(
                "Pray4Chichester", 
                "Oops!  Newtwork error, please try that again.", 
                parent=parent)
    return
...
class BlueButton(Button):
    ...
    # Initialisation code
    def __init__(self, parent, column, row, *argv, 
                 text=None, command=None, padx=5, pady=5, sticky=(E, W), 
                 **args):
        super().__init__(parent, *argv, text=text,
                         bg=blue, fg='white', command=command, **args)
...
class Card(Toplevel):
    ...
    # Initialisation code
    def __init__(self, prayerwall, 
                 prayer=(0, "A Prayer", 0), x=0, y=0, wrap=3, offset=False):
        ...
        label = Label(heading, textvariable=self.subject,
                      font=self.bold, wraplength=int(self.width/2))
        ...
        self.zoom = BlueButton(heading, text="Zoom out",
                               command=self.doZoom, row=0, column=2)

        # row 1 prayer text
        self.prayer = ScrolledText(
            frame, font=self.normal, height=5, wrap=WORD)
        ...
        self.countFrame = LabelFrame(
            frame, text="Prayed 0 times", font=self.normal)
        self.countFrame.grid(
            row=2, column=0, columnspan=2, sticky=(E, W, N, S))
        self.coloured.append(self.countFrame)  # so we can modify bg ...
        # add prayed and response and maybe new request buttons
        button1 = BlueButton(self.countFrame, text="I've prayed",
                             command=self.doPrayed, row=1, column=1)
        button2 = BlueButton(self.countFrame, text="I want to respond",
                             command=self.newResponse, row=1, column=2)
        ...
        self.responsesFrame = LabelFrame(
            frame, text="Responses", font=self.normal)
        self.responsesFrame.grid(
            row=3, column=1, columnspan=3, sticky=(E, W, N, S))
        ...
        self.responseItems = ScrolledText(
            self.responsesFrame, font=self.normal, height=5, wrap=WORD)
        ...
        self.timestamp = 0
        # process any set-up as an update
        self.update(prayer, x, y, wrap, offset)
        return
    ...
    def setup(self, x, y, wrap=3, offset=False):
        ...
        # print(f"[{self.counter}] Setup: "
        #       "width = {self.width}, height = {self.height}")
        self.x = w * x + edge
        if offset:
            self.x += WIDTH
        self.y = h * y + edge
        # time since last move - so we don't disturb a card being read
        ...
        if diff > 60 * 5:  # five minutes
            ...
            # print("deiconifying from:", self.state())
            self.state('normal')
            geometry = f'{self.width}x{self.height}+{self.x}+{self.y}'
            # print(f"Setting g to: "
    ...
    def aChange(self, event):
        ...
        # a size match - might not match position
        if (event.width == intW) and (event.height == intH):
            # if got bigger and not already reacted - zoom font
            if (intW > (self.width * 2.5) and 
                intH > (self.height * 2.5) and 
                self.normal.cget("size") == fontSize):
            ...
            if (intW < (self.width * 1.5) and 
                intH < (self.height * 1.5) and 
                self.normal.cget("size") != fontSize):
    ...
    def newResponse(self):
        ...
        dialog = PrayerDialog(self, title="Your Response",
                              mustHaveSubject=False)
        if dialog.result:
            subject, response, author = dialog.result
            ok = prayPost("add", number=self.number,
                          postData=(subject, response, author))
            if ok:
                self.refresh()
                messagebox.showinfo("Pray4Chichester",
    ...
    def doPrayed(self):
        ...
        if ok:
            ...
            messagebox.showerror("Pray4Chichester", 
                                 "Oops!  Newtwork error, please try that again.", 
                                 parent=self)
        return
    ...
    def setCount(self, count):
        if count == 0:
            ...
            self.countFrame.config(
                text="Has been prayed for " + str(count) + " times")
    ...
    def setResponses(self, responses):
        ...
        self.responseItems.tag_config(
            "separator", background="pink", foreground="pink")
...
class PrayerWall(Tk):
    ...
    def __init__(self, name, col=0, row=0, **args):
        ...
        button = Button(self.frame, text="New Prayer Request",
                        bg=blue, command=self.newRequest)
